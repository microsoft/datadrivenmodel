...
class QubeSimulator(object):
    ...
    def reset(self, config=None):
        ...
        if config:
            ...
            self.state = np.array([
                config.get("config_initial_theta", np.random.uniform(-0.27, 0.27)),
                config.get("config_initial_alpha", np.random.uniform(-0.05, 0.05)),
                config.get("config_initial_theta_dot", np.random.uniform(-0.05, 0.05)),
                config.get("config_initial_alpha_dot", np.random.uniform(-0.05, 0.05)),
            ])
        return self.state
